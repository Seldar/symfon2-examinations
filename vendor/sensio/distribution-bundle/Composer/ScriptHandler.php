...
EOF
    ...
    protected static function updateDirectoryStructure(Event $event, $rootDir, $appDir, $binDir, $varDir, $webDir)
    {
        ...
        foreach (array(
            ...
            $appDir.'/phpunit.xml' => $rootDir.'/phpunit.xml',
        ) as $source => $target) {
...
EOF;
        $phpunit = str_replace(array('<directory>../src', '"bootstrap.php.cache"', $phpunitKernelBefore), array('<directory>src', '"'.$varDir.'/bootstrap.php.cache"', $phpunitKernelAfter), file_get_contents($rootDir.'/phpunit.xml'));
        ...
        $fs->dumpFile($rootDir.'/phpunit.xml', $phpunit);
